...
# PHASE 4
First, integrate `Donchian_Bands.mq5` into `Custom_Indicator.mq5`.
...
For a candle crossing a border (i.e., open is on one side of the border and close is on the other side):
...
- "beyond (bullish) agreeing border" means `candle close > (bullish) agreeing border`, and "within (bullish) agreeing border" means `candle close < (bullish) agreeing border`.
- "beyond (bullish) disagreeing border" means `candle close < (bullish) disagreeing border`, and "within (bullish) disagreeing border" means `candle close > (bullish) disagreeing border`.

Mirror logic for (bearish) agreeing/disagreeing border:
- "beyond (bearish) agreeing border" means `candle close < (bearish) agreeing border`, and "within (bearish) agreeing border" means `candle close > (bearish) agreeing border`.
- "beyond (bearish) disagreeing border" means `candle close > (bearish) disagreeing border`, and "within (bearish) disagreeing border" means `candle close < (bearish) disagreeing border`.

---

# PHASE 5
Once the borders and basic interactions are defined, proceed to define push events and candle-border interactions with active locks.
