...
1.  **Bearish Push Counter-cross**:
    *   Current Bearish Push is active (`bearPushState.active`).
...
1.  **Bullish Push Counter-cross**:
    *   Current Bullish Push is active (`bullPushState.active`).
...
*   **Logic**:
    1.  **Detection**: Monitors for `up > prevUp` or `down < prevDown`. Validates candle direction and internal border crossing count (max 1 for cross push).
